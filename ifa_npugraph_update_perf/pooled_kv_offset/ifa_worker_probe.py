...
def main() -> int:
    ...
    if os.environ.get("IFA_KV_PACKED") in ("1", "2"):
        del keys, values
        kv_pool = torch.zeros((2, args.records, num_blocks, page_size, num_kv_heads * head_dim), device=device, dtype=dtype)
        keys = [kv_pool[0, i] for i in range(args.records)]
        values = [kv_pool[1, i] for i in range(args.records)]
        if os.environ.get("IFA_KV_PACKED") == "2":
            def rebase(t):
                begin = t.storage_offset() * t.element_size()
                storage = t.untyped_storage()[begin:begin + t.numel() * t.element_size()]
                out = torch.empty(0, dtype=t.dtype, device=t.device).set_(storage, 0, t.shape, t.stride())
                assert out.data_ptr() == t.data_ptr() and out.storage_offset() == 0
                return out
            keys = [rebase(t) for t in keys]
            values = [rebase(t) for t in values]
    ...
    if os.environ.get("IFA_SEQ_PATTERN") == "ragged":
        updates[0]["actual_seq_lengths_kv"] = [args.seq_len + 1 + (i % 3) for i in range(args.batch_size)]
    result["seq_pattern"] = os.environ.get("IFA_SEQ_PATTERN", "uniform")
    result["kv_packed"] = os.environ.get("IFA_KV_PACKED", "0")
    result["seq_progress"] = os.environ.get("IFA_SEQ_PROGRESS", "0")
    ...
    worker_mode = os.environ.get("IFA_WORKER_MODE", "pool")
    class FreshExecutor:
        def submit(self, fn):
    ...
    result["worker_mode"] = worker_mode
    ...
    def timed_update() -> float:
        ...
        if os.environ.get("IFA_SEQ_PROGRESS") == "1":
            update_counter += 1
            updates[0]["actual_seq_lengths_kv"] = [args.seq_len + update_counter] * args.batch_size
        started = time.perf_counter_ns()
        with torch.profiler.record_function("IFA_DIAG_UPDATE"):
